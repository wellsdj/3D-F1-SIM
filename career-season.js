...
function migrate(c){
 ...
 c.history=Array.isArray(c.history)?c.history:[];c.archive=Array.isArray(c.archive)?c.archive:[];
...
function lap(c,time){migrate(c);const w=c.weekend;if(!Number.isFinite(time)||time<=0||!['practice','qualifying'].includes(w.stage))return false;
 w[w.stage].push(time);if(w.stage==='practice'&&w.practice.length===1){c.coins+=250;w.practiceReward=250;}return true;}
...
function score(c,rows){migrate(c);const w=c.weekend;if(w.stage!=='race'||w.scored||!rows.some(r=>r.you))return false;
 ...
 c.coins+=prize+bonus;c.reputation=clamp(c.reputation+(objective?8:2)-(me.dsq?5:0),0,100);
 c.last={season:c.season,round:c.round,pos:mine+1,points:pts,prize,bonus,objective,dsq:!!me.dsq,dnf:!!me.dnf,grid:w.gridPosition||10,rows:rows.map(r=>({who:r.who,you:!!r.you,dsq:!!r.dsq,dnf:!!r.dnf,total:r.total||null}))};
...
return {migrate,event,begin,lap,advance,qualify,score,standings,nextSeason,POINTS};
});
